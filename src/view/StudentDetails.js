...
class StudentDetails extends Component {
    ...
    detailView(item) {
        ...
        // })
        var raw = "";
        var url = "https://620502d5161670001741b2f7.mockapi.io/staff/student/"
        var requestOptions = {
            method: 'DELETE',
            body: raw,
            redirect: 'follow'
        ...
        return (
            <SafeAreaView style={{ flex: 1, }}>
                ...
                <LinearGradient style={styles.oval} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} colors={['#1e6a78', '#1e6a78', '#1e6a78']}>
                    ...
                    <TouchableOpacity style={styles.backTouchView}
                        onPress={() => {
                            this.props.navigation.navigate('StudentsCreate')
                        }}
                    >
                        <Image
    ...
    renderItem = ({ item }) => {
        ...
        return (
            <View style={{ backgroundColor: constant.WHITE, margin: 5, borderRadius: 10, padding: 10 }}
            onPress={() => {
                this.navv(item)
            }}
            >
                ...
                <View style={{}}>
                    ...
                    <View style={{ flexDirection: 'row', marginTop: hp('4%'), marginLeft: hp('.5%') }}>
                        <Text style={styles.sampleType}>Student Name:</Text>
                        <Text style={styles.sampleType1}>{item.student_name}</Text>
                    </View>

                    <View style={{ flexDirection: 'row', marginTop: hp('-1%'), marginLeft: hp('.5%') }}>
                        <Text style={styles.sampleType}>Father Name</Text>
                        <Text style={styles.sampleType}>:</Text>
                        <Text style={styles.sampleType1}>{item.Father_name}</Text>
                    </View>
                    ...
                    <View style={{ flexDirection: 'row', marginTop: hp('-1%'), marginLeft: hp('.5%') }}>
                        <Text style={styles.sampleType}>Department   :</Text>
                        <Text style={styles.sampleType1}>{item.department}</Text>
                    </View>

                    <View style={{ flexDirection: 'row', marginTop: hp('-1%'), marginLeft: hp('.5%') }}>
                        <Text style={styles.sampleType}>Joining Date   :</Text>
                        <Text style={styles.sampleType1}>{item.joining_date}</Text>
                    </View>

                    <View style={{ flexDirection: 'row', marginTop: hp('-1%'), marginLeft: hp('.5%') }}>
                        <Text style={styles.sampleType}>Aadhar Number No  :</Text>
                        <Text style={styles.sampleType1}>{item.addharcard_no}</Text>
                    </View>


                    <View style={{ flexDirection: 'row', marginTop: hp('-1%'), marginLeft: hp('.5%') }}>
                        <Text style={styles.sampleType}>Address   :</Text>
                        <Text style={styles.sampleType1}>{item.address}</Text>
                    </View>
...
const styles = StyleSheet.create({
    ...
    ViewList: {
        height: hp("3%"), width: hp("3%"), alignSelf: 'center', marginTop: hp('.5%'), borderRadius: 5, marginLeft: (Platform.OS === 'android' ? hp('24%') : hp('35%')),
    },
    searchTextView: {
    ...
    listView: {
        flex: 1, backgroundColor: "transparent", marginTop: hp("2%")
    },
});
